...
| `mist_power_level` | Випаровування |
| `mist_power_level = low` | низька інтенсивність |
| `mist_power_level = medium` | середня інтенсивність |
| `mist_power_level = high` | висока інтенсивність |
...
`Режим` — це назва UI-параметра, а `денний авто` — значення, зібране з `runtime` і `mode`.
`Випаровування` — це назва UI-параметра потужності пароутворювача, а `середня інтенсивність` — значення, отримане з `mist_power_level`.
