...
| 26 | GPIO4 | Light output | output | active-low switched output; `0` = ON, `1` = OFF; safe state = OFF | `LIGHT_GPIO` |
| 29 | GPIO5 | Status LED green | output | active-low LED; `0` = ON, `1` = OFF | `STATUS_LED_GREEN_GPIO` |
| 16 | GPIO13 | Main fan output | output | active-low switched output; `0` = ON, `1` = OFF; safe state = OFF | `MAIN_FAN_GPIO` |
| 23 | GPIO15 | Status LED red | output | active-low LED; `0` = ON, `1` = OFF; boot strap pin | `STATUS_LED_RED_GPIO` |
| 31 | GPIO19 | Humidifier local fan output | output | active-low switched output; `0` = ON, `1` = OFF; safe state = OFF | `HUMIDIFIER_FAN_GPIO` |
...
| 11 | GPIO26 | Humidifier mist control | output | active-low switched output; `0` = ON, `1` = OFF; safe state = OFF | `HUMIDIFIER_MIST_GPIO` |
...
All actuator outputs are active-low and must initialize to OFF:

```text
GPIO4  LIGHT_GPIO            OFF = 1
GPIO13 MAIN_FAN_GPIO         OFF = 1
GPIO19 HUMIDIFIER_FAN_GPIO   OFF = 1
GPIO26 HUMIDIFIER_MIST_GPIO  OFF = 1
```
